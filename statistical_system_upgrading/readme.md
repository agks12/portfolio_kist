...
통계조사 홈페이지의 시각화 서비스를 안정적으로 제공하기 위해서는 다수의 이종 원천 데이터(Excel, GIS 등)를 일정한 스키마로 가공하여 데이터 마트에 정확하게 적재하는 과정이 필수적입니다. 

하지만 기존 환경에서는 다음과 같은 한계와 운영상의 어려움이 존재했습니다.

* **수동 관리 및 파이프라인 정합성 한계:** 기존에 구축된 Airflow 파이프라인을 운영하는 과정에서 간헐적인 DAG 로직 오류나 적재 누락이 발생할 경우, 이를 사전에 탐지하고 즉각적으로 대응할 수 있는 시스템적 검증 장치가 부족했습니다.
* **원천 데이터 품질 불확실성:** 납품받은 엑셀 원본 데이터 자체에 오탈자나 잘못된 값이 포함되어 있거나, 이종 포맷 간 상이한 조건(버전 및 배치 패턴 등)으로 인해 데이터 왜곡이 발생할 위험이 상존했습니다.
* **긴급 데이터 정합성 유지의 필요성:** 시각화 대시보드로 데이터가 송출되기 전, 오류가 발견되더라도 DB 상에서 신속하게 원인을 분석하고 보정할 수 있는 체계적인 모니터링 및 품질 관리 프로세스의 확립이 요구되었습니다.
